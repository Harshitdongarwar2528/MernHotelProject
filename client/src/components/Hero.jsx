...
const Hero = () => {
    ...
    const onSearch = async (e) => {
        ...
        // Add destination to recent searched cities max 3 recent searched cities
        setSearchedCities((prevSearchedCities) => {
            const updatedSearchedCities = [...prevSearchedCities, destination];
            if (updatedSearchedCities.length > 3) {
                updatedSearchedCities.shift(); // Remove the oldest search
}
  ...
  return (
    <div className='flex flex-col items-start justify-center px-6 md:px-16 lg:px-24 xl:px-32 text-white bg-[url("/src/assets/heroImage.png")] bg-no-repeat bg-cover bg-center h-screen'>
      ...
      <p className='max-w-130px mt-2 text-sm md:text-base'>Unparalleded luxary and comfort await at the world's most exclusive <br/> hotels and resorts.
         ...
         <form onSubmit={onSearch} className='bg-white text-gray-500 rounded-lg px-6 py-4 mt-8 flex flex-col md:flex-row max-md:items-start gap-4 max-md:mx-auto'>
            ...
            <div>
                <div className='flex items-center gap-2'>
                    <img src={assets.calenderIcon} alt="clendar" className='h-4'/>
