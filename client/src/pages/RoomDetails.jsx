...
//check if the room is available
  const checkAvailability = async(e)=>{
    try{
      ...
      if(checkInDate >= checkOutDate){
        toast.error("Check-In date must be Lesser than Check-Out date");
        return;
      }
  ...
  // onSubmitHandler function to check availibilty and book the room
...
<span>{room.hotelAddress || room.hotel.address
       ...
       <div className="grid grid-cols-2 gap-4 lg:w-1/2 w-full h-[400px]">
    {room?.images.length > 1 &&
      room.images.map((image, index) => (
        ...
        >
          <img
            ...
            className={`w-full h-full object-cover ${
              mainImage === image && "outline-orange-500 outline-4"
            }`}
          />
        </div>
      ))}
        ...
        <form  onSubmit={onSubmitHandler} className="flex flex-col md:flex-row items-start md:items-center justify-between bg-white shadow-[0px_0px_20px_rgba(0,0,0,0.15)] p-6 rounded-xl mx-auto mt-16 max-w-6xl">
          ...
          <button type="submit" className="bg-blue-500 hover:bg-blue-600 active:scale-95 transition-all text-white rounded-md max-md:w-full max-md:mt-6 md:px-12 py-3 text-base cursor-pointer">
